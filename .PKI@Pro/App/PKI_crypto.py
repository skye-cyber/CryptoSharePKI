...
WATCHED_DIR = os.path.abspath(get_value('WATCH_DIR'))  # Path to the mounted shared folder
# Path to the public key directory
PUBLIC_KEY_STORE = os.path.abspath(get_value('PUBLIC_KEY_STORE'))
PRIVATE_KEY_PATH = os.path.abspath(os.path.join(get_value('PRIVATE_KEY_PATH')), f"{HOST}.pem")  # Path to the private key for decryption
ENCRYPTED_EXTENSION = os.path.abspath(get_value('ENCRYPTED_EXTENSION'))  # Extension for encrypted files
